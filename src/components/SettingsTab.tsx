...
export const SettingsTab: React.FC = () => {
  ...
  return (
    <div className={`rounded-xl p-4 md:p-6 border transition-colors shadow-sm space-y-4 ${
      ...
      <h2 className={`font-bold text-lg md:text-xl flex items-center space-x-2 border-b pb-3 ${isLight ? 'border-slate-200' : 'border-slate-700'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
        ...
        <div className="space-y-4">
          ...
          <div className={`p-3.5 rounded-xl border space-y-2 ${
            ...
            <label className={`block text-xs font-bold ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>介面主題 (日間 / 夜間模式)</label>
              ...
              <button
                ...
                className={`p-2.5 rounded-lg border flex items-center justify-center space-x-2 text-xs font-bold transition ${
                  themeMode === 'dark'
                    ...
                    : (isLight ? 'bg-white border-slate-300 text-slate-700' : 'bg-slate-800 border-slate-700 text-slate-400')
                }`}
              >
          ...
          <div className={`p-3.5 rounded-xl border space-y-2 ${
            ...
            <label className={`block text-xs font-bold ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>預設券商手續費折數 (折)</label>
            <input
              type="number"
              step="0.01"
            ...
            />
            <p className={`text-[11px] ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>如：3.8折請填 0.38；0 折免手續費請填 0</p>
          </div>

          <div className={`p-3.5 rounded-xl border space-y-2 ${
            isLight ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/80 border-slate-700'
          }`}>
            <label className={`block text-xs font-bold ${isLight ? 'text-slate-700' : 'text-slate-300'}`}>投資上限金額設定 (NT$)</label>
            <input
              type="number"
            ...
            <div className="flex space-x-2">
              <button
                ...
                className={`flex-1 p-2.5 rounded-lg text-xs font-bold flex justify-center items-center space-x-1 transition ${
                  isLight ? 'bg-white hover:bg-slate-100 text-slate-800 border border-slate-300' : 'bg-slate-700 hover:bg-slate-600 text-white'
                }`}
              >
          ...
          <div className={`pt-2 ${isLight ? 'border-slate-200' : 'border-slate-700'}`}>
            <button
